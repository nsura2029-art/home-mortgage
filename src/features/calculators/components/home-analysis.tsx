        ...
        <div className="grid lg:grid-cols-[0.88fr_1.12fr]">
          <div className="border-b border-slate-200 p-6 sm:p-9 lg:border-b-0 lg:border-r">
            <div className="mb-8 flex items-center justify-between">
              <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-blue">Your home snapshot</p><h2 className="mt-2 text-2xl font-bold tracking-tight">Start with what you know.</h2></div>
          ...
          <div className="relative min-h-[590px] bg-surface p-6 sm:p-9">
            <div className="absolute right-0 top-0 size-56 rounded-full bg-blue-100/60 blur-3xl" />
            <AnimatePresence mode="wait">
              {!showResults ? (
                <motion.div key="preview" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="relative flex h-full min-h-[500px] flex-col justify-between">
                  <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-400">Live estimate</p><div className="mt-5 rounded-3xl border border-white bg-white/80 p-6 shadow-sm backdrop-blur"><p className="text-sm text-slate-500">Estimated monthly payment</p><p className="mt-2 text-4xl font-extrabold tracking-tight">{usd.format(results.totalMonthlyPayment)}<span className="text-base font-medium text-slate-400"> / mo</span></p><div className="mt-6 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full w-[72%] rounded-full bg-blue" /></div><div className="mt-4 flex justify-between text-xs text-slate-500"><span>Principal & interest</span><span>Taxes, insurance & PMI</span></div></div></div>
                  <div className="rounded-3xl bg-navy p-6 text-white"><div className="flex size-10 items-center justify-center rounded-xl bg-white/10"><Sparkles className="size-5 text-emerald" /></div><p className="mt-5 text-lg font-semibold">There may be more in your mortgage than a monthly payment.</p><p className="mt-2 text-sm leading-6 text-slate-300">We’ll look for savings, PMI milestones, equity, and your refinance break-even point.</p></div>
                </motion.div>
              ) : (
...
function Insight({ icon: Icon, label, value, detail, accent = false }: { icon: typeof Home; label: string; value: string; detail: string; accent?: boolean }) {
  return <div className={`rounded-2xl border p-5 ${accent ? "border-emerald-200 bg-emerald-50/70" : "border-slate-200 bg-white"}`}><div className="flex items-center gap-2 text-xs font-semibold text-slate-500"><Icon className={`size-4 ${accent ? "text-emerald-600" : "text-blue"}`} />{label}</div><p className="mt-3 text-2xl font-extrabold tracking-tight">{value}</p><p className="mt-1 text-xs leading-5 text-slate-500">{detail}</p></div>;
}
